...
import { Target, Trophy, Filter, Download, Calendar, DollarSign, List, Truck, Calculator } from 'lucide-react';
...
export default function SellersTab({ sellers, settings }) {
  ...
  // --- 2. CÁLCULOS (LÓGICA DO HTML) ---
  const totals = useMemo(() => {
    return detailData.sales.reduce((acc, curr) => ({
      netRevenue: acc.netRevenue + curr.revenue,      // Venda Pedra (Base Comissão)
      freight: acc.freight + (curr.freight || 0),     // Frete (Repasse)
      m2: acc.m2 + curr.m2_total
    }), { netRevenue: 0, freight: 0, m2: 0 });
  }, [detailData.sales]);

  const grossRevenue = totals.netRevenue + totals.freight; // Total Nota
  ...
  return (
    <div className="space-y-6 fade-in pb-20">
      ...
      {/* --- SEÇÃO 1: CABEÇALHO DE FILTROS E META --- */}
      ...
      {loading ? (
        ...
        <>
          {/* --- SEÇÃO 2: BARRA DE TOTAIS (ESTILO RELATÓRIO) --- */}
          ...
          {/* --- SEÇÃO 3: EXTRATO DETALHADO (TABELA PRINCIPAL) --- */}
              ...
              <table className="w-full text-xs text-left whitespace-nowrap">
                ...
                <tbody className="divide-y divide-slate-100 text-slate-600">
                   {detailData.sales.length === 0 ? (
                      <tr><td colSpan="8" className="p-8 text-center text-slate-400">Nenhuma venda encontrada neste período.</td></tr>
                   ) : (
                      detailData.sales
                        .sort((a,b) => new Date(a.date) - new Date(b.date)) // Ordem Cronológica (como extrato)
                        .map((sale, i) => (
                         <tr key={i} className="hover:bg-slate-50 transition-colors">
                            <td className="p-3">{new Date(sale.date).toLocaleDateString()}</td>
                            <td className="p-3 font-medium text-slate-800 truncate max-w-[200px]" title={sale.client}>{sale.client}</td>
                            ...
                            <td className="p-3 text-right text-orange-600 bg-orange-50/30">{sale.freight > 0 ? formatBRL(sale.freight) : '-'}</td>
                            <td className="p-3 text-right font-bold text-slate-900">{formatBRL(sale.revenue + sale.freight)}</td>
                         </tr>
                      ))
                   )}
                </tbody>
