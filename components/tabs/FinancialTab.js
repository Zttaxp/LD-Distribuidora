...
import { Settings, FileMinus, Plus, Trash2, RefreshCw, Loader2, Scale, AlertTriangle, TrendingUp } from 'lucide-react'; // Novos ícones importados
...
export default function FinancialTab({ summary, settings, expenses, onSettingsChange, onExpensesChange }) {
  // ... (ESTADOS e USEEFFECTS ANTERIORES PERMANECEM IGUAIS - NÃO ALTERAR) ...
  ...
  const sysGross = sysNetRev + sysFreight;
  const sysCMV = Number(currentSummary.total_cost) || 0;
  
  // --- NOVOS DADOS DE METRAGEM ---
  ...
  // --- NOVA LÓGICA: UNIT ECONOMICS (CUSTO POR M²) ---
  // Custo Operacional Total = Despesas Fixas + Variáveis (Não inclui CMV nem impostos sobre venda, pois estamos analisando custo da estrutura)
  // Mas para saber "quem paga a conta", o ideal é ver quanto de Margem de Contribuição cada M² gera vs o Custo Fixo por M².
  
  const totalOpExpenses = sysFixedExp + sysVarExp;
  const costPerM2 = m2Total > 0 ? totalOpExpenses / m2Total : 0; // Quanto cada m² precisa pagar de conta de luz/salário/aluguel

  // Receita Média por Tipo (estimada baseada no mix)
  // O banco não traz receita separada por tipo na view atual, mas podemos inferir ou sugerir atualização futura.
  // Por enquanto, vamos assumir que o "High" custa > 300/m2 e "Low" < 300/m2 na venda.
  // Vamos criar um indicador de "Ponto de Equilíbrio Operacional"
  
  const breakEvenM2 = (sysNetRev - sysCMV - sysTaxes - sysComms) > 0 
    ? totalOpExpenses / ((sysNetRev - sysCMV - sysTaxes - sysComms) / m2Total) 
    : 0;

  // --- HANDLERS E UTILS (MANTIDOS) ---
  const handleSaveSettings = async () => { /* ... igual código anterior ... */ setLoading(true); try { await updateSettings(localSettings); onSettingsChange(localSettings); alert('Salvo!'); } catch (e) { alert(e.message); } finally { setLoading(false); } };
  const handleAddExpense = async () => { /* ... igual código anterior ... */ if (!newExpense.name || !newExpense.value) return; setLoading(true); try { const payload = { ...newExpense, month_key: newExpense.type === 'VARIABLE' ? selectedMonth : null }; const { data } = await addExpense(payload); onExpensesChange([...expenses, data]); setNewExpense({ name: '', value: '', type: 'FIXED' }); } catch (e) { alert(e.message); } finally { setLoading(false); } };
  const handleDeleteExpense = async (id) => { /* ... igual código anterior ... */ if(!confirm('Apagar?')) return; try { await deleteExpense(id); onExpensesChange(expenses.filter(e => e.id !== id)); } catch (e) { alert(e.message); } };
  const handleManualUpdate = async (field, value) => { /* ... igual código anterior ... */ const newData = { month_key: selectedMonth, gross_revenue: manGross, freight: manFreight, cmv: manCMV, tax_rate: manRateTax, comm_rate: manRateComm, fixed_expenses: manFixedExp, variable_expenses: manVarExp, ...{ [field]: Number(value) } }; setManualData(prev => ({ ...prev, ...newData })); await saveManualScenario(newData); };
  const handleResetManual = async () => { /* ... igual código anterior ... */ if(!confirm("Resetar?")) return; setManualData(null); };
  ...
  return (
    <div className="space-y-6 fade-in pb-10">
      {/* 1. INPUTS E CONTROLES (MANTIDO) */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* ... (Blocos de Parâmetros e Despesas - CÓDIGO IGUAL AO ANTERIOR, MANTER AQUI) ... */}
        {/* Vou resumir para focar na novidade, mas no seu código mantenha os inputs completos */}
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
           ...
           <div className="grid grid-cols-3 gap-4">
              {/* Inputs de Imposto, Comissão, Inadimplência aqui... */}
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
           {/* Bloco de Adicionar Despesa aqui... */}
      ...
      {/* --- NOVA SEÇÃO: UNIT ECONOMICS (QUEM PAGA A CONTA?) --- */}
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        <h4 className="font-bold text-slate-700 flex items-center gap-2 mb-4 border-b border-slate-100 pb-2">
          <Scale size={18} className="text-cyan-600" />
          Eficiência Operacional: Quem paga a conta?
        </h4>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Card 1: Custo Operacional por M² */}
          <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 flex flex-col items-center justify-center text-center">
            <span className="text-xs font-bold text-slate-500 uppercase mb-1">Custo Op. (Despesas) por M²</span>
            <span className="text-2xl font-extrabold text-red-600">{formatBRL(costPerM2)}</span>
            <span className="text-[10px] text-slate-400 mt-1">Cada m² serrado precisa pagar isso só de estrutura</span>
          </div>

          {/* Card 2: Análise de Carga - Alto Valor */}
          <div className="bg-purple-50 p-4 rounded-lg border border-purple-100 flex flex-col justify-center">
            <div className="flex justify-between items-center mb-2">
              <span className="text-xs font-bold text-purple-700 uppercase">Alto Valor ({m2High.toFixed(0)} m²)</span>
              <span className="text-[10px] bg-purple-200 text-purple-800 px-1 rounded">Volume: {m2Total > 0 ? ((m2High/m2Total)*100).toFixed(0) : 0}%</span>
            </div>
            <p className="text-xs text-slate-600 mb-1">
              Contribuição Estimada: <strong className="text-purple-700">Alta</strong>
            </p>
            ...
            <p className="text-[10px] text-purple-500 mt-2 italic">
              Este grupo paga as contas e gera o lucro real da empresa.
            </p>
          </div>

          {/* Card 3: Análise de Carga - Baixo Valor */}
          <div className="bg-orange-50 p-4 rounded-lg border border-orange-100 flex flex-col justify-center">
            <div className="flex justify-between items-center mb-2">
              <span className="text-xs font-bold text-orange-700 uppercase">Baixo Valor ({m2Low.toFixed(0)} m²)</span>
              <span className="text-[10px] bg-orange-200 text-orange-800 px-1 rounded">Volume: {m2Total > 0 ? ((m2Low/m2Total)*100).toFixed(0) : 0}%</span>
            </div>
            <p className="text-xs text-slate-600 mb-1">
              Risco: Preço Médio vs Custo de {formatBRL(costPerM2)}/m²
            </p>
            {/* Alerta se o custo fixo for alto demais para o material barato */}
            <div className="flex items-start gap-2 mt-2 bg-white p-2 rounded border border-orange-100">
               <AlertTriangle size={14} className="text-orange-500 shrink-0 mt-0.5" />
               <p className="text-[10px] text-slate-500 leading-tight">
                 Se você vender material barato abaixo de <strong>{formatBRL(sysCMV/m2Total + costPerM2)}</strong>/m², você está pagando para trabalhar.
               </p>
            </div>
          </div>
        </div>
      ...
      {/* DRE E GRÁFICOS (MANTIDOS) */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
         {/* ... (DRE System e Manual - CÓDIGO IGUAL AO ANTERIOR, MANTER AQUI) ... */}
         {/* Vou incluir apenas o Wrapper para você saber onde fica */}
         <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 shadow-inner">
            ...
            <div className="space-y-3 text-sm">
               ...
               <DreRowInput label="(-) CMV" value={sysCMV} pct={sysNetRev ? sysCMV/sysNetRev*100 : 0} isRed />
               {/* ... restante do DRE System ... */}
         ...
         <div className="bg-white p-6 rounded-xl border-2 border-indigo-100 shadow-sm relative">
            ...
            <div className="space-y-3 text-sm">
                {/* Inputs manuais... */}
                <DreInput label="(+) Rec. Bruta" val={manGross} onChange={v => handleManualUpdate('gross_revenue', v)} isBold />
                <DreInput label="(-) Fretes" val={manFreight} onChange={v => handleManualUpdate('freight', v)} isRed isSmall />
                {/* ... restante dos inputs ... */}
                <div className="flex justify-between border-t border-indigo-50 pt-1 font-semibold text-indigo-700 bg-indigo-50 px-1 rounded"><span>(=) Rec. Líquida</span><span>{formatBRL(manNetRev)}</span></div>
