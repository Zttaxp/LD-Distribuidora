...
export default function FinancialTab({ summary, settings, expenses, onSettingsChange, onExpensesChange }) {
  ...
  const sysNetRev = Number(currentSummary.total_net_revenue) || 0; // Venda Pedra
  const sysFreight = Number(currentSummary.total_freight) || 0;    // Frete
  ...
  const manRevenue = manualData?.gross_revenue ?? sysNetRev;
  const manFreight = manualData?.freight ?? sysFreight;
  const manGross = manRevenue + manFreight;
    ...
    <div className="space-y-6 fade-in pb-10">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
           <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2 text-sm"><Settings size={16} /> Parâmetros (Aplicados na Venda Pedra)</h3>
           <div className="grid grid-cols-3 gap-4">
              <div><label className="text-[10px] uppercase text-slate-700 font-bold">Impostos %</label><input type="number" value={localSettings.tax_rate} onChange={e => setLocalSettings({...localSettings, tax_rate: e.target.value})} onBlur={handleSaveSettings} className="w-full border-slate-300 rounded text-sm p-1.5 text-slate-900 font-bold" /></div>
              <div><label className="text-[10px] uppercase text-slate-700 font-bold">Comissão %</label><input type="number" value={localSettings.comm_rate} onChange={e => setLocalSettings({...localSettings, comm_rate: e.target.value})} onBlur={handleSaveSettings} className="w-full border-slate-300 rounded text-sm p-1.5 text-slate-900 font-bold" /></div>
              <div><label className="text-[10px] uppercase text-red-700 font-bold">Inadimplência %</label><input type="number" value={localSettings.bad_debt_rate} onChange={e => setLocalSettings({...localSettings, bad_debt_rate: e.target.value})} onBlur={handleSaveSettings} className="w-full border-slate-300 rounded text-sm p-1.5 text-slate-900 font-bold" /></div>
           </div>
        </div>
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
           <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2 text-sm"><FileMinus size={16} /> Adicionar Despesa</h3>
           <div className="flex gap-2 mb-3">
              <input type="text" placeholder="Nome" value={newExpense.name} onChange={e => setNewExpense({...newExpense, name: e.target.value})} className="w-1/3 border-slate-300 rounded text-xs p-2 text-slate-900 font-medium" />
              <input type="number" placeholder="R$" value={newExpense.value} onChange={e => setNewExpense({...newExpense, value: e.target.value})} className="w-1/4 border-slate-300 rounded text-xs p-2 text-slate-900 font-medium" />
              <select value={newExpense.type} onChange={e => setNewExpense({...newExpense, type: e.target.value})} className="w-1/4 border-slate-300 rounded text-xs p-2 text-slate-900 font-medium"><option value="FIXED">Fixa</option><option value="VARIABLE">Variável</option></select>
           ...
           <div className="max-h-24 overflow-y-auto space-y-1 custom-scroll">
              {currentExpenses.map(exp => (<div key={exp.id} className="flex justify-between p-2 bg-slate-50 rounded text-xs border border-slate-200"><span className="text-slate-700 font-bold">{exp.name}</span><div className="flex gap-2 font-bold"><span className="text-slate-800">{formatBRL(exp.value)}</span><button onClick={() => handleDeleteExpense(exp.id)} className="text-red-600 hover:text-red-800"><Trash2 size={12}/></button></div></div>))}
           </div>
        </div>
      </div>

      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 shadow-sm relative z-10">
        <div className="flex items-center gap-4"><h3 className="font-bold text-slate-800 text-lg">Comparativo Financeiro</h3><select value={selectedMonth} onChange={e => setSelectedMonth(e.target.value)} className="bg-slate-100 border-none rounded text-sm font-bold text-slate-900 py-1 px-3 cursor-pointer">{months.map(m => <option key={m.key} value={m.key}>{m.label}</option>)}</select></div>
        <button onClick={handleResetManual} className="text-xs bg-indigo-50 text-indigo-700 px-3 py-1.5 rounded-lg font-bold border border-indigo-100 hover:bg-indigo-100"><RefreshCw size={12} /> Resetar</button>
      </div>

      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        <h4 className="font-bold text-slate-800 flex items-center gap-2 mb-4 border-b border-slate-100 pb-2"><Scale size={18} className="text-cyan-700" /> Eficiência Operacional</h4>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 flex flex-col items-center justify-center text-center">
            <span className="text-xs font-bold text-slate-700 uppercase mb-1">Custo Op. por M²</span><span className="text-2xl font-extrabold text-red-700">{formatBRL(costPerM2)}</span><span className="text-[10px] text-slate-500 mt-1 font-medium">Gasto fixo para produzir 1 m²</span>
          </div>
          <div className="bg-purple-50 p-4 rounded-lg border border-purple-100 flex flex-col justify-center">
            <div className="flex justify-between items-center mb-2"><span className="text-xs font-bold text-purple-800 uppercase">Alto Valor ({m2High.toFixed(0)} m²)</span><span className="text-[10px] bg-purple-200 text-purple-900 px-1 rounded font-bold">Vol: {m2Total > 0 ? ((m2High/m2Total)*100).toFixed(0) : 0}%</span></div>
            <div className="w-full bg-purple-200 h-1.5 rounded-full mt-1"><div className="bg-purple-700 h-1.5 rounded-full" style={{width: '90%'}}></div></div>
          </div>
          <div className="bg-orange-50 p-4 rounded-lg border border-orange-100 flex flex-col justify-center">
             <div className="flex justify-between items-center mb-2"><span className="text-xs font-bold text-orange-800 uppercase">Baixo Valor ({m2Low.toFixed(0)} m²)</span><span className="text-[10px] bg-orange-200 text-orange-900 px-1 rounded font-bold">Vol: {m2Total > 0 ? ((m2Low/m2Total)*100).toFixed(0) : 0}%</span></div>
             <div className="flex items-start gap-2 mt-2 bg-white p-2 rounded border border-orange-100"><AlertTriangle size={14} className="text-orange-600 shrink-0 mt-0.5" /><p className="text-[10px] text-slate-600 leading-tight font-medium">Custo Operacional: <strong className="text-slate-900">{formatBRL(costPerM2)}</strong>/m²</p></div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
         ...
         <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 shadow-inner">
            <h4 className="font-bold text-slate-700 uppercase text-xs mb-4 text-center border-b border-slate-200 pb-2">Dados do Sistema (Realizado)</h4>
            <div className="space-y-3 text-sm">
               ...
               <div className="flex justify-between border-t border-b border-slate-200 py-2 font-bold text-blue-900 bg-blue-100 px-2 rounded">
               ...
               <div className="pl-2 border-l-2 border-slate-300 space-y-1 mt-2">
               ...
               <div className="flex justify-between font-bold text-orange-800 mt-2 bg-orange-100 p-1 rounded border border-orange-200"><span>(=) Margem Contrib.</span><span>{formatBRL(sysMargin)}</span></div>
               ...
               <div className="flex justify-between p-3 bg-white rounded-lg border-2 border-green-200 mt-4 shadow-sm"><span className="font-extrabold text-green-900">LUCRO LÍQUIDO</span><div className="flex flex-col items-end"><span className={`font-extrabold text-lg ${sysProfit >= 0 ? 'text-green-800' : 'text-red-700'}`}>{formatBRL(sysProfit)}</span><span className="text-[10px] font-bold text-green-700">{formatPct(sysProfitPct)}</span></div></div>
            </div>
         </div>

         {/* DRE SIMULADO */}
         <div className="bg-white p-6 rounded-xl border-2 border-indigo-100 shadow-sm relative">
            <div className="absolute top-0 right-0 bg-indigo-100 text-indigo-800 text-[10px] font-bold px-2 py-1 rounded-bl-lg">EDITÁVEL</div>
            <h4 className="font-bold text-indigo-800 uppercase text-xs mb-4 text-center border-b border-indigo-100 pb-2">Análise Manual</h4>
            <div className="space-y-3 text-sm">
                <div className="flex justify-between items-center text-xs text-slate-500 font-medium">
                   <span>Total Nota (Calc):</span>
                   <span className="text-slate-700 font-bold">{formatBRL(manGross)}</span>
                </div>
                <DreInput label="(-) Fretes" val={manFreight} onChange={v => handleManualUpdate('freight', v)} isRed isSmall />
                
                <div className="flex justify-between border-t border-b border-indigo-100 py-2 font-bold text-indigo-900 bg-indigo-50 px-2 rounded">
                  <span>(=) Fat. Bruto (Pedra)</span>
                  <div className="flex items-center gap-2">
                     <span className="text-[10px] text-indigo-500 font-bold">Base</span>
                ...
                <div className="flex justify-between font-bold text-orange-800 mt-2 px-1 bg-orange-50 rounded border border-orange-100"><span>(=) Margem Contrib.</span><span>{formatBRL(manMargin)}</span></div>
...
// Helpers com Cores Mais Fortes
function DreRow({ label, value, isBold, isRed, isSmall }) { 
    const formatBRL = (val) => new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(val); 
    return (
        <div className={`flex justify-between items-center ${isSmall ? 'text-xs text-slate-700' : ''}`}>
            <span className={isBold ? 'text-slate-800 font-bold' : ''}>{label}</span>
            <span className={`font-bold ${isRed ? 'text-red-600' : 'text-slate-900'}`}>{formatBRL(value)}</span>
        </div>
    ); 
}
...
function DreRowInput({ label, value, pct, isRed, isSmall }) { 
    const formatBRL = (val) => new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(val); 
    return (
        <div className={`flex justify-between items-center ${isSmall ? 'text-xs text-slate-700' : ''} ${isRed ? 'bg-red-50 p-1 rounded border border-red-200' : ''}`}>
            <span className="font-medium">{label}</span>
            <div className="flex gap-2">
                <span className={`text-[10px] px-1 rounded self-center font-bold ${isRed ? 'bg-red-100 text-red-800' : 'bg-slate-200 text-slate-700'}`}>{pct.toFixed(1)}%</span>
                <span className={isRed ? 'text-red-700 font-bold' : 'text-slate-800 font-bold'}>{formatBRL(value)}</span>
            </div>
        </div>
    ); 
}
...
function DreInput({ label, val, onChange, isBold, isRed, isSmall }) { 
    return (
        <div className={`flex justify-between items-center ${isSmall ? 'text-xs' : ''} ${isRed ? 'bg-red-50 p-1 rounded border border-red-200' : ''}`}>
            <span className={`${isBold ? 'text-slate-800 font-bold' : 'text-slate-700 font-medium'} ${isRed ? 'text-red-800' : ''}`}>{label}</span>
            <input type="number" value={val || 0} onChange={e => onChange(e.target.value)} className={`input-clean text-right bg-transparent border-b border-slate-400 w-24 focus:ring-0 focus:border-indigo-600 ${isRed ? 'text-red-800 font-bold' : 'text-indigo-900 font-bold'} ${isSmall ? 'text-xs' : ''} text-slate-900`} />
        </div>
    ); 
}
...
function DreInputRate({ label, val, onChange, displayVal }) { 
    ...
    return (
        <div className="flex justify-between items-center text-xs">
            <span className="text-slate-700 font-medium">{label} (<input type="number" value={val} onChange={e => onChange(e.target.value)} className="w-8 text-center border-b border-slate-400 text-[10px] bg-transparent focus:outline-none focus:border-indigo-600 text-slate-900 font-bold" />%)</span>
            <span className="text-red-600 font-bold">{formatBRL(displayVal)}</span>
        </div>
    ); 
}