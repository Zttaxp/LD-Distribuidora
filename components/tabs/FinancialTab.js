...
import { Settings, FileMinus, Plus, Trash2, RefreshCw, Loader2, Scale, AlertTriangle } from 'lucide-react';
...
export default function FinancialTab({ summary, settings, expenses, onSettingsChange, onExpensesChange }) {
  ...
  // --- HANDLERS (CORRIGIDO ADD EXPENSE) ---
  const handleSaveSettings = async () => { 
    setLoading(true); 
    const res = await updateSettings(localSettings); 
    setLoading(false);
    if(res.success) { onSettingsChange(localSettings); alert('Salvo!'); } 
  ...
  const handleAddExpense = async () => {
    ...
    // Chamada Segura para o Server Action
    const res = await addExpense(payload);
    setLoading(false);

    if (res.success) {
      onExpensesChange([...expenses, res.data]);
      setNewExpense({ name: '', value: '', type: 'FIXED' });
    } else {
      alert(res.message); // Mostra o erro sem quebrar a tela
    }
  };
  ...
  const handleDeleteExpense = async (id) => { 
    ...
    if(res.success) onExpensesChange(expenses.filter(e => e.id !== id));
    else alert(res.message);
  };

  const handleManualUpdate = async (field, value) => { 
    const newData = { month_key: selectedMonth, gross_revenue: manGross, freight: manFreight, cmv: manCMV, tax_rate: manRateTax, comm_rate: manRateComm, fixed_expenses: manFixedExp, variable_expenses: manVarExp, ...{ [field]: Number(value) } }; 
    setManualData(prev => ({ ...prev, ...newData })); 
    await saveManualScenario(newData); 
  };

  const handleResetManual = async () => { if(!confirm("Resetar?")) return; setManualData(null); };
  ...
  return (
    <div className="space-y-6 fade-in pb-10">
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        <h4 className="font-bold text-slate-700 flex items-center gap-2 mb-4 border-b border-slate-100 pb-2"><Scale size={18} className="text-cyan-600" /> Eficiência Operacional: Quem paga a conta?</h4>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 flex flex-col items-center justify-center text-center">
            <span className="text-xs font-bold text-slate-500 uppercase mb-1">Custo Op. (Despesas) por M²</span><span className="text-2xl font-extrabold text-red-600">{formatBRL(costPerM2)}</span><span className="text-[10px] text-slate-400 mt-1">Cada m² serrado precisa pagar isso só de estrutura</span>
          </div>
          <div className="bg-purple-50 p-4 rounded-lg border border-purple-100 flex flex-col justify-center">
            <div className="flex justify-between items-center mb-2"><span className="text-xs font-bold text-purple-700 uppercase">Alto Valor ({m2High.toFixed(0)} m²)</span><span className="text-[10px] bg-purple-200 text-purple-800 px-1 rounded">Volume: {m2Total > 0 ? ((m2High/m2Total)*100).toFixed(0) : 0}%</span></div>
          ...
          <div className="bg-orange-50 p-4 rounded-lg border border-orange-100 flex flex-col justify-center">
            <div className="flex justify-between items-center mb-2"><span className="text-xs font-bold text-orange-700 uppercase">Baixo Valor ({m2Low.toFixed(0)} m²)</span><span className="text-[10px] bg-orange-200 text-orange-800 px-1 rounded">Volume: {m2Total > 0 ? ((m2Low/m2Total)*100).toFixed(0) : 0}%</span></div>
