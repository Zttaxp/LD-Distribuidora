...
export default function AnnualTab({ summary, settings, expenses, scenarios }) {
  ...
  // 2. Processamento Contábil Rigoroso (DRE)
  const annualData = useMemo(() => {
    ...
    for (let i = 1; i <= 12; i++) {
      ...
      // --- LEITURA DOS DADOS (CORREÇÃO APLICADA AQUI) ---
      // Lemos o Líquido e o Frete do banco para compor o Bruto
      ...
      // --- CÁLCULO DRE (PADRÃO CONTÁBIL) ---
      const netRev = grossRev - freight;           // 1. Receita Líquida
      const taxes = netRev * (taxRate / 100);      // 2. Impostos
      const commissions = netRev * (commRate / 100); // 3. Comissões
      const badDebt = netRev * (badDebtRate / 100);  // 4. Inadimplência
      
      const deductions = taxes + commissions + badDebt; 
      
      const margin = netRev - cmv - deductions;    // 5. Margem de Contribuição
      const totalExpenses = fixedExp + varExp;     // 6. Despesas Operacionais
      
      const profit = margin - totalExpenses;       // 7. Lucro Líquido (Resultado)
      ...
      months.push({
        ...
        profit,
        profitMargin,
        hasData: isManualMode ? (sysData || hasManualData) : !!sysData
      });
    }
    return months;
  ...
  const chartData = {
    labels: annualData.map(d => d.monthName.substr(0, 3)),
    datasets: [{
    ...
    <div className="space-y-6 fade-in pb-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-3 bg-slate-50 p-1.5 rounded-lg border border-slate-200">
            <div className="flex items-center px-2">
              ...
              <select value={viewMode} onChange={(e) => setViewMode(e.target.value)} className="bg-transparent border-none text-sm font-bold text-purple-800 cursor-pointer focus:ring-0 py-0">
            ...
            <div className="flex items-center px-2">
              ...
              <select value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} className="bg-transparent border-none text-sm font-bold text-purple-800 cursor-pointer focus:ring-0 py-0">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <h3 className="font-bold text-slate-700 mb-4 text-sm uppercase tracking-wide flex justify-between items-center">
          ...
          <span className={`text-[10px] px-2 py-1 rounded ${viewMode === 'MANUAL' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>Modo: {viewMode === 'MANUAL' ? 'Simulação Manual' : 'Realizado'}</span>
        </h3>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
          ...
          <button onClick={() => window.print()} className="text-xs flex items-center gap-1 text-slate-500 hover:text-slate-800 font-medium"><Printer size={14} /> Imprimir Relatório</button>
        </div>
            ...
            <tbody className="divide-y divide-slate-100 text-slate-600">
              {annualData.map((d, i) => (
                <tr key={i} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="p-3 text-slate-900 bg-blue-50/30 font-medium">{d.grossRev > 0 ? formatBRL(d.grossRev) : '-'}</td>
              ...
              <tr className="bg-slate-100 font-bold text-slate-800 border-t-2 border-slate-200 text-sm">
                ...
                <td className={`p-3 border-l border-slate-300 text-base ${totals.prof >= 0 ? 'text-green-800 bg-green-200' : 'text-red-800 bg-red-200'}`}>{formatBRL(totals.prof)}</td>
